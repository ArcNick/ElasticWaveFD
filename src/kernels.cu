...
__global__ void update_stress_coarse(Core core, Model model, PsiVel psi_vel, int cur) {
    ...
    if (tex1Dfetch<int>(mat_tex, iz * nx + ix) == SOLID) {
        ...
        if (tex1Dfetch<int>(txz_mask, iz * (nx - 1) + ix) == -1) {
            float dvz_dx = dvx_dz_coarse(core.vx, ix, iz, cur ^ 1);
            float dvx_dz = dvz_dx_coarse(core.vz, ix, iz, cur ^ 1);
...
__global__ void update_velocity_coarse(Core core, Model model, PsiStr psi_str, int cur) {
    ...
    if (tex1Dfetch<int>(mat_tex, iz * nx + ix) == SOLID) {
        ...
        float dp_dx = dsx_dx_coarse(core.sx, ix, iz, cur);
        float dp_dz = dsz_dz_coarse(core.sz, ix, iz, cur);
        core.vx[IdxVxCo(ix, iz, cur)] = core.vx[IdxVxCo(ix, iz, cur ^ 1)] + (
            + dt_d / samp_rho_x_coarse(model.rho, ix, iz) * dp_dx
        );
        core.vz[IdxVzCo(ix, iz, cur)] = core.vz[IdxVzCo(ix, iz, cur ^ 1)] + (
            + dt_d / samp_rho_z_coarse(model.rho, ix, iz) * dp_dz
        );
    }
}
...
__global__ void update_velocity_fine(Core core, Model model, int cur, int zone) {
    ...
    if (tex1Dfetch<int>(mat_tex, IdxSigFi(ix, iz, 0, zone)) == SOLID) {
        if (ix < fines[zone].lenx - 1) {
            float dtxz_dz = dtxz_dz_8th(core.txz, ix, iz, cur, zone);
            float dsx_dx = dsx_dx_8th(core.sx, ix, iz, cur, zone);
        ...
        if (iz < fines[zone].lenz - 1) {
            float dtxz_dx = dtxz_dx_8th(core.txz, ix, iz, cur, zone);
            float dsz_dz = dsz_dz_8th(core.sz, ix, iz, cur, zone);
        ...
        if (ix < fines[zone].lenx - 1) {
            float dp_dx = dsx_dx_8th(core.p, ix, iz, cur, zone);
            core.vx[IdxVxFi(ix, iz, cur, zone)] = core.vx[IdxVxFi(ix, iz, cur ^ 1, zone)] + (
               - dt_d * dp_dx / samp_rho_x_fine(model.rho, ix, iz, zone)
            );
        }

        if (iz < fines[zone].lenz - 1) {
            float dp_dz = dsz_dz_8th(core.p, ix, iz, cur, zone);
            core.vz[IdxVzFi(ix, iz, cur, zone)] = core.vz[IdxVzFi(ix, iz, cur ^ 1, zone)] + (
                - dt_d * dp_dz / samp_rho_z_fine(model.rho, ix, iz, zone)
            );
        }
    }
}
