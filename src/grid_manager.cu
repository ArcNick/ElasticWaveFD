...
void GridManager::load_from_file(const std::string &file) {
    ...
    // fine
    for (int i = 0, offset = nx_coarse * nz_coarse; 
        i < num; i++, offset += fine_info[i].lenx * fine_info[i].lenz) {
        for (int j = 0; j < 5; j++) {
            item = fine_ptr;
...
void GridManager::build_mask() {
    /* 这里的掩码只是为了方便对粗细网格更新使用不同的核函数
    ** 对于求 n 没有起到决定性帮助
    ** 求 n 需要遍历细网格区判断到细网格边界的距离
    ** 这里仅对粗网格进行创建掩码
    */ 
    ...
    // 核心修改：改为 Linear 类型（匹配 cudaMalloc 分配的内存）
    ...
    cudaMemcpyToSymbol(sx_mask, &texObj, sizeof(cudaTextureObject_t));
    ...
    cudaCreateTextureObject(&texObj, &resDesc, &texDesc, NULL);
    cudaMemcpyToSymbol(sz_mask, &texObj, sizeof(cudaTextureObject_t));
    cudaDestroyTextureObject(texObj);
    ...
    cudaCreateTextureObject(&texObj, &resDesc, &texDesc, NULL);
    cudaMemcpyToSymbol(vx_mask, &texObj, sizeof(cudaTextureObject_t));
    cudaDestroyTextureObject(texObj);
    ...
    cudaCreateTextureObject(&texObj, &resDesc, &texDesc, NULL);
    cudaMemcpyToSymbol(vz_mask, &texObj, sizeof(cudaTextureObject_t));
    cudaDestroyTextureObject(texObj);
    ...
    cudaCreateTextureObject(&texObj, &resDesc, &texDesc, NULL);
    cudaMemcpyToSymbol(txz_mask, &texObj, sizeof(cudaTextureObject_t));
    cudaDestroyTextureObject(texObj);
...
void GridManager::build_n() {
    ...
    */ 
    ...
    cudaCreateTextureObject(&texObj, &resDesc, &texDesc, NULL);
    cudaMemcpyToSymbol(vx_n_tex, &texObj, sizeof(cudaTextureObject_t));
    cudaDestroyTextureObject(texObj);
    ...
    cudaCreateTextureObject(&texObj, &resDesc, &texDesc, NULL);
    cudaMemcpyToSymbol(vz_n_tex, &texObj, sizeof(cudaTextureObject_t));
    cudaDestroyTextureObject(texObj);
    ...
    cudaCreateTextureObject(&texObj, &resDesc, &texDesc, NULL);
    cudaMemcpyToSymbol(sx_n_tex, &texObj, sizeof(cudaTextureObject_t));
    cudaDestroyTextureObject(texObj);
    ...
    cudaCreateTextureObject(&texObj, &resDesc, &texDesc, NULL);
    cudaMemcpyToSymbol(sz_n_tex, &texObj, sizeof(cudaTextureObject_t));
    cudaDestroyTextureObject(texObj);
    ...
    cudaCreateTextureObject(&texObj, &resDesc, &texDesc, NULL);
    cudaMemcpyToSymbol(txz_n_tex, &texObj, sizeof(cudaTextureObject_t));
    cudaDestroyTextureObject(texObj);
...
void GridManager::build_constant() {
    ...
    for (int i = 1; i < fine_info.size(); i++) {
...
void GridManager::memory_allocate() {
    ...
    // FD_n
    cudaMalloc((void**)&vx_n, (nx_coarse - 1) * nz_coarse * sizeof(FD_n));
    cudaMalloc((void**)&vz_n, nx_coarse * (nz_coarse) * sizeof(FD_n));
    cudaMalloc((void**)&sx_n, nx_coarse * nz_coarse * sizeof(FD_n));
    cudaMalloc((void**)&sz_n, nx_coarse * nz_coarse * sizeof(FD_n));
    cudaMalloc((void**)&txz_n, (nx_coarse - 1) * (nz_coarse - 1) * sizeof(FD_n));
}
...
void GridManager::memory_release() {
    ...
    // mask(texture)
    if (core_mask.vx) {
        cudaFree(core_mask.vx);
        core_mask.vx = nullptr;
    }
    if (core_mask.vz) {
        cudaFree(core_mask.vz);
        core_mask.vz = nullptr;
    }
    if (core_mask.sx) {
        cudaFree(core_mask.sx);
        core_mask.sx = nullptr;
    }
    if (core_mask.sz) {
        cudaFree(core_mask.sz);
        core_mask.sz = nullptr;
    }
    if (core_mask.txz) {
        cudaFree(core_mask.txz);
        core_mask.txz = nullptr;
    }
    ...
    // device
    if (model_d.rho) {
        cudaFree(model_d.rho);
        model_d.rho = nullptr;
    }
    if (model_d.C11) {
        cudaFree(model_d.C11);
        model_d.C11 = nullptr;
    }
    if (model_d.C13) {
        cudaFree(model_d.C13);
        model_d.C13 = nullptr;
    }
    if (model_d.C33) {
        cudaFree(model_d.C33);
        model_d.C33 = nullptr;
    }
    if (model_d.C55) {
        cudaFree(model_d.C55);
        model_d.C55 = nullptr;
    }
